...
const SemesterExamPage4 = () => {
  ...
  const fetchCourses = () => {
    userApi.subscription.getAllCourses({
      setIsLoading,
      params: {
        semester: semesterId,
      },
  ...
  const fetchCourseSubjectsByCourseId = courseId => {
    userApi.courses.getById({
      params: { courseCategoryId: semesterId },
      setIsLoading,
      onSuccess: res => {
        if (!res?.data?.length) {
          showNotification({
              ...
              {courses?.map((course, index) => (
                <div key={index} className="bg-green-100 p-4 rounded-lg shadow text-center">
                  <p className="text-center bg-green-300 p-1 rounded inline-block">
                    <img
                      src={course?.courseImage?.[0] || course.icon}
                      alt={course.title}
...
export default HOC(SemesterExamPage4);
