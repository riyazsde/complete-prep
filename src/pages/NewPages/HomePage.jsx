...
const HomePage = () => {
    ...
    userApi.landingPage.getTopBanner({
      ...
      onSuccess: data => {
        const banners = (data?.data || []).filter(
          banner => banner?.status === true && banner?.image
        );
  ...
  const fetchGoal = async () => {
    userApi.universityCourse.getAll({
      params: {
        universityId: selectedGoalCategory,
      },
  ...
  const fetchFaqs = async () => {
    userApi.faq.getAll({
      onSuccess: data => {
        setFaqs(
          (data?.data || []).filter(item => item?.showOnHomepage)
        );
      },
  ...
  return (
    <>
      ...
      <Helmet>
        <title>
          Semprep | Smarter Study Resources for Semester Exams
        </title>
                ...
                >
                  {banner?.image && (
                    <img
                      ...
                      className="block w-full h-auto max-h-[420px] object-contain object-center rounded-2xl"
                    />
                  )}
                </div>
              ))}
        ...
        <div>
          ...
          <div className="flex lg:flex-row flex-col-reverse gap-10 lg:gap-5 items-center bg-black lg:h-[calc(100vh-80px)] py-10 relative">
            ...
            <div className="flex flex-col lg:items-start justify-center text-center md:px-6 px-3 mb-4 z-10 flex-1 lg:absolute">
              ...
              <p className="text-sm md:text-base text-white lg:text-start py-3 max-w-[400px]">
                Semprep is a one stop solution for all the
                higher education students to prepare for
          ...
          >
            <p className="text-zinc-800 text-lg fw-semibold">
              The trusted learning companion for students
              across India
            </p>
          ...
          >
            ...
            <p className="text-center text-gray-500 mt-2 text-sm md:text-base max-w-xl mx-auto">
              See how Capcable has transformed users’
              social experiences through their own words.
            </p>
